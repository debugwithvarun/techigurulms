...
import { Play, ArrowRight, MonitorPlay, Users, Award, Clock, Sparkles, Globe, TrendingUp, BookOpen } from 'lucide-react';
...
const AnimatedStat = ({ value, suffix = '', label, icon: Icon, color }) => {
  ...
  useEffect(() => {
    if (!inV) return;
    const step = num / (1800 / 16); let cur = 0;
    const t = setInterval(() => {
      cur = Math.min(cur + step, num); setCount(Math.floor(cur));
  ...
  return (
    <motion.div ref={ref}
      ...
      className="group relative flex flex-col items-center text-center p-6 bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg hover:shadow-purple-100/60 transition-all duration-300 overflow-hidden"
    >
      {/* Hover top accent */}
      <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-purple-500 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
      ...
      <div className="text-3xl font-extrabold text-slate-900 mb-1">{count.toLocaleString()}{suffix}</div>
...
const LearningSection = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (d?.courses) setShowcaseCourses(d.courses.slice(0, 3));
    })();
  }, []);
  ...
  const features = [
    { icon: MonitorPlay, title: 'HD Video Lessons',   desc: 'Crystal-clear video with offline support' },
    { icon: Globe,       title: 'Learn Anywhere',     desc: 'Desktop, tablet, or mobile access' },
    { icon: TrendingUp,  title: 'Track Progress',     desc: 'Visual milestones and achievements' },
    { icon: Sparkles,    title: 'Expert Mentors',     desc: 'India\'s top industry professionals' },
  ];
    ...
    <section className="overflow-hidden bg-slate-50">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <div className="inline-flex items-center gap-2 bg-indigo-50 text-indigo-700 text-xs font-bold px-4 py-2 rounded-full mb-6 border border-indigo-100">
              <Sparkles size={12} /> India's Leading EdTech Platform
            </div>
            <h2 className="text-4xl lg:text-5xl font-extrabold text-slate-900 leading-[1.15] mb-6 tracking-tight">
              Learn From{' '}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-purple-600">Top Experts</span>,<br />
              Anytime & Anywhere
            </h2>
            <p className="text-lg text-slate-500 leading-relaxed mb-8 max-w-lg">
              Master in-demand skills with India's most comprehensive learning platform — structured courses, real-world projects, and industry-recognized certificates.
            </p>
            ...
            <div className="flex flex-wrap gap-4">
              <button onClick={() => navigate('/active-course')}
                ...
                Explore Courses <ArrowRight size={15} className="group-hover:translate-x-1 transition-transform" />
              </button>
              <button className="flex items-center gap-2 bg-white hover:bg-slate-50 text-slate-700 font-bold px-7 py-3.5 rounded-xl text-sm border border-slate-200 shadow-sm transition-all">
                <Play size={14} className="text-indigo-600" fill="currentColor" /> Watch Demo
              </button>
            </div>

            <div className="mt-8 flex items-center gap-4">
              <div className="flex -space-x-2.5">
                {[{ i: 'RS', b: 'bg-purple-600' }, { i: 'AK', b: 'bg-indigo-600' }, { i: 'PM', b: 'bg-emerald-600' }, { i: 'SG', b: 'bg-orange-500' }]
                  .map((a, i) => (
                    <div key={i} className={`w-8 h-8 rounded-full border-2 border-white flex items-center justify-center text-white text-[10px] font-black ${a.b}`}>{a.i}</div>
                  ))}
              </div>
              <div>
                ...
                <div className="flex items-center gap-0.5 mt-0.5">
                  ...
                  <span className="text-xs text-slate-500 ml-1.5 font-medium">4.9 avg rating</span>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          <div className="relative flex justify-center lg:justify-end">
                ...
                <div className="bg-white/90 backdrop-blur-md rounded-2xl p-4 shadow-xl border border-white/60">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">Full Stack Dev · 68% complete</p>
                    </div>
                    <div className="ml-auto w-10 h-10 rounded-full border-4 border-indigo-100 flex items-center justify-center">
            ...
            <motion.div
              initial={{ opacity: 0, x: 20, y: 20 }} whileInView={{ opacity: 1, x: 0, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.25 }}
            ...
            >
              <img src="https://images.unsplash.com/photo-1522202176988-66273c2fd55f?q=80&w=600&auto=format&fit=crop"
                alt="Collaboration" className="w-full h-full object-cover" />
            </motion.div>

            <motion.button
              initial={{ scale: 0 }} whileInView={{ scale: 1 }} whileHover={{ scale: 1.12 }} viewport={{ once: true }}
      ...
      <div className="py-20 px-6 lg:px-12 max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
          <h2 className="text-3xl lg:text-4xl font-extrabold text-slate-900 mb-4">Everything You Need to Succeed</h2>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {features.map((f, i) => (
            <motion.div key={i}
              ...
              className="group bg-white rounded-2xl border border-slate-100 p-7 hover:shadow-xl hover:shadow-purple-100/50 hover:-translate-y-1 transition-all duration-300"
            >
              <div className="w-12 h-12 bg-indigo-50 rounded-xl flex items-center justify-center mb-5 group-hover:bg-indigo-600 transition-colors duration-300">
                <f.icon size={21} className="text-indigo-600 group-hover:text-white transition-colors duration-300" />
              </div>
        ...
        <div className="bg-slate-900 py-20 px-6 lg:px-12">
          <div className="max-w-7xl mx-auto">
            <div className="flex items-end justify-between mb-10">
              <div>
                <p className="text-indigo-400 text-xs font-bold mb-2 uppercase tracking-widest">Popular Now</p>
                ...
                return (
                  ...
                  >
                    <div className="h-44 overflow-hidden bg-slate-800">
                      <img src={thumb} alt={course.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 opacity-90" />
                    </div>
                    <div className="p-5">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="flex items-center gap-1 text-xs text-slate-400"><Users size={11} /> {course.studentsEnrolled || 0}</span>
      ...
      <div className="bg-white py-14 px-6 lg:px-12 border-t border-slate-100">
        <div className="max-w-7xl mx-auto text-center">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-8">Our students work at India's top companies</p>
          <div className="flex flex-wrap justify-center items-center gap-8 lg:gap-16">
            {['Infosys', 'TCS', 'Wipro', 'Reliance', 'Amazon', 'Flipkart'].map((c, i) => (
              <motion.span key={i} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}
                className="text-xl lg:text-2xl font-black text-slate-300 hover:text-slate-600 transition-colors cursor-default select-none">
                {c}
              </motion.span>
            ))}
          </div>
