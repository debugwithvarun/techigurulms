...
import {
    Users, FileText, CheckCircle, Clock, Video,
...
const PROGRESS_OPTIONS = [
    { value: 'below_avg', label: 'Below Average', icon: ArrowDown, color: 'text-red-600 bg-red-50 border-red-200' },
    { value: 'avg',       label: 'Average',       icon: Minus,     color: 'text-amber-600 bg-amber-50 border-amber-200' },
    { value: 'above_avg', label: 'Above Average', icon: ArrowUp,   color: 'text-green-600 bg-green-50 border-green-200' },
];
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }} onClick={onClose}>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <h3 className="font-black text-gray-900">Assign Task to {intern.fullName}</h3>
...
const MeetModal = ({ interns, onClose }) => {
    ...
    const toggleIntern = (id) => setSelected(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);
    ...
    const handleSend = async () => {
        ...
        setLoading(true);
        try {
            await api.post('/internship/send-meet', { meetLink, title, scheduledAt, internIds: selected });
            setSent(true);
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }} onClick={onClose}>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                ...
                onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <h3 className="font-black text-gray-900 flex items-center gap-2"><Video size={18} className="text-blue-600" /> Send Meet Link</h3>
                    <button onClick={onClose} className="p-2 rounded-xl hover:bg-gray-100 text-gray-400"><X size={16} /></button>
                </div>
                {sent ? (
                    <div className="p-8 text-center">
                        ...
                        <p className="font-bold text-gray-900 mb-1">Meet link sent to {selected.length} intern(s)!</p>
                        <button onClick={onClose} className="mt-4 px-6 py-2.5 bg-gray-100 rounded-xl text-sm font-bold text-gray-700">Close</button>
                    </div>
                ) : (
                    <div className="p-6 space-y-4">
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Meeting Title *</label>
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Meet Link *</label>
                            <input value={meetLink} onChange={e => setMeetLink(e.target.value)}
                                placeholder="https://meet.google.com/..."
                                className="w-full px-4 py-3 text-sm border border-gray-200 rounded-xl outline-none focus:border-blue-500" />
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Scheduled At</label>
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex gap-2">
                                    <button onClick={selectAll} className="text-xs font-bold text-blue-600 hover:underline">All</button>
                                    <button onClick={clearAll}  className="text-xs font-bold text-gray-400 hover:underline">Clear</button>
                                </div>
                            </div>
                            <div className="max-h-36 overflow-y-auto space-y-1.5 border border-gray-200 rounded-xl p-3">
                                {interns.map(i => (
                                    <label key={i._id} className="flex items-center gap-2.5 cursor-pointer">
                                        <input type="checkbox" checked={selected.includes(i._id)} onChange={() => toggleIntern(i._id)} className="accent-blue-600" />
                        ...
                        <button onClick={handleSend} disabled={loading || !meetLink || !title || selected.length === 0}
...
// ── Intern Detail Panel ────────────────────────────────────────────────────────
const InternPanel = ({ intern, tasks, tickets, onClose, onTaskAssigned, onMarkProgress, onReviewTask, onResolveTicket }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex justify-end" style={{ background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)' }} onClick={onClose}>
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 26 }}
                ...
                <div className="flex items-center justify-between px-6 py-5 bg-gradient-to-r from-purple-600 to-indigo-600">
                    <div>
                        ...
                        <p className="text-purple-200 text-xs mt-0.5">{intern.role} · {intern.college}</p>
                    </div>
                    ...
                    {SECTIONS.map(s => (
                        <button key={s.id} onClick={() => setActiveSection(s.id)}
                            className={`px-4 py-3 text-xs font-bold border-b-2 -mb-px transition-colors
                                ${activeSection === s.id ? 'border-purple-600 text-purple-700' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
                            {s.label}
                        </button>
                    ))}
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-5">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold
                                                ...
                                                : task.status === 'revision_needed' ? 'bg-red-100 text-red-700'
                                                : 'bg-gray-100 text-gray-600'}`}>
                                                {task.status.replace('_', ' ')}
                                            </span>
                                            {task.dueDate && <span className="text-[10px] text-gray-400">{new Date(task.dueDate).toLocaleDateString('en-IN')}</span>}
                                        </div>
                                        {task.status === 'submitted' && (
                                            <div className="flex gap-1.5">
                                                <button onClick={() => onReviewTask(task._id, 'approved', '')}
                                                    className="px-2.5 py-1 bg-green-600 text-white text-[10px] font-bold rounded-lg hover:bg-green-700">Approve</button>
                                                <button onClick={() => { setTaskToReview(task._id); }}
                                                    className="px-2.5 py-1 bg-orange-100 text-orange-700 text-[10px] font-bold rounded-lg hover:bg-orange-200">Revision</button>
                                            </div>
                                        )}
                                    </div>
                                    {task.submissionUrl && (
                                        <a href={task.submissionUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 font-semibold hover:underline mt-1 block">
                                            View Submission →
                                        </a>
                                    )}
                                    {taskToReview === task._id && (
                                        <div className="mt-3 flex gap-2">
                                            <input value={feedback} onChange={e => setFeedback(e.target.value)}
                                                placeholder="Feedback for revision..." className="flex-1 px-3 py-1.5 text-xs border border-gray-200 rounded-lg outline-none" />
                                            <button onClick={() => { onReviewTask(task._id, 'revision_needed', feedback); setTaskToReview(null); setFeedback(''); }}
                                                className="px-3 py-1.5 bg-orange-500 text-white text-xs font-bold rounded-lg">Send</button>
                                        </div>
                                    )}
                                </div>
                            ))}
                    ...
                    {activeSection === 'progress' && (
                        <div className="space-y-4">
                            <p className="text-sm text-gray-500">Mark weekly performance for <strong>{intern.fullName}</strong>.</p>
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Week Number</label>
                                <input type="number" min={1} max={52} value={progressWeek} onChange={e => setProgressWeek(e.target.value)}
                                    className="w-full px-4 py-3 text-sm border border-gray-200 rounded-xl outline-none focus:border-purple-500" />
                            </div>
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Performance Rating</label>
                                <div className="grid grid-cols-3 gap-2">
                                    {PROGRESS_OPTIONS.map(opt => (
                                        <button key={opt.value} onClick={() => setProgressRating(opt.value)}
                                            className={`flex flex-col items-center gap-1.5 py-3 rounded-xl border-2 transition-all text-xs font-bold
                                                ${progressRating === opt.value ? opt.color + ' border-current' : 'border-gray-200 text-gray-400 hover:border-gray-300'}`}>
                                            <opt.icon size={16} />
                                            {opt.label}
                                        </button>
                                    ))}
                            ...
                            <div className="space-y-2">
                                ...
                                <textarea rows={3} value={progressRemarks} onChange={e => setProgressRemarks(e.target.value)}
                                    placeholder="Detailed feedback about this week's performance..."
                                    className="w-full px-4 py-3 text-sm border border-gray-200 rounded-xl outline-none focus:border-purple-500 resize-none" />
                            </div>
                            <button onClick={() => onMarkProgress(intern._id, progressRating, progressRemarks, progressWeek)}
                    ...
                    {activeSection === 'tickets' && (
                        <div className="space-y-3">
                            {tickets.length === 0 ? (
                                ...
                                <div key={t._id} className="bg-white border border-gray-100 rounded-xl p-4 shadow-sm space-y-2">
                                    <div className="flex items-start justify-between">
                                        <p className="font-bold text-gray-900 text-sm">{t.subject}</p>
                                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ml-2 shrink-0
                                    ...
                                    {t.status === 'open' && (
                                        <div className="flex gap-2">
                                            <input value={resolutionMap[t._id] || ''} onChange={e => setResolutionMap(p => ({ ...p, [t._id]: e.target.value }))}
                                                placeholder="Resolution note..."
                                                className="flex-1 px-3 py-1.5 text-xs border border-gray-200 rounded-lg outline-none" />
                                            <button onClick={() => onResolveTicket(t._id, resolutionMap[t._id] || '')}
                                                className="px-3 py-1.5 bg-green-600 text-white text-xs font-bold rounded-lg hover:bg-green-700">Resolve</button>
                                        </div>
                                    )}
                                    {t.resolution && <p className="text-xs text-green-700 bg-green-50 px-3 py-2 rounded-lg">✓ {t.resolution}</p>}
                                </div>
                            ))}
                        </div>
                    )}
...
const SubHRDashboard = () => {
    ...
    const loadAll = useCallback(async () => {
        ...
        try {
            ...
            const tasksMap = {}, ticketsMap = {};
            await Promise.all(internData.map(async (intern) => {
                const [tasksRes, ticketsRes] = await Promise.all([
                    api.get(`/internship/tasks/intern/${intern._id}`).catch(() => ({ data: [] })),
                    api.get('/internship/tickets/subhr').catch(() => ({ data: [] })),
                ]);
                tasksMap[intern._id] = tasksRes.data || [];
                ticketsMap[intern._id] = (ticketsRes.data || []).filter(t =>
                    t.application?.toString() === intern._id?.toString() || t.raisedBy?._id === intern.applicant?._id
                );
            }));
    ...
    const handleMarkProgress = async (applicationId, rating, remarks, week) => {
        try {
            await api.post('/internship/progress', { applicationId, rating, remarks, week });
    ...
    const handleReviewTask = async (taskId, status, feedbackNote) => {
        try {
            await api.put(`/internship/tasks/${taskId}/review`, { status, feedbackNote });
            loadAll();
        } catch (err) { alert(err.response?.data?.message || 'Failed'); }
    };
    ...
    const sidebarContent = (
        <div className="flex flex-col h-full" style={{ background: '#1c1d1f' }}>
            <div className="flex items-center gap-3 px-5 py-5 border-b border-white/10">
                ...
                <button onClick={() => setSidebarOpen(false)} className="ml-auto lg:hidden text-gray-400"><X size={18} /></button>
            </div>
            <div className="px-4 py-4 border-b border-white/10">
                <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full flex items-center justify-center font-bold text-white text-sm bg-blue-600">{user?.name?.[0]?.toUpperCase()}</div>
                ...
                {TABS.map(tab => {
                    ...
                    return (
                        <button key={tab.id} onClick={() => { setActiveTab(tab.id); setSidebarOpen(false); }}
                            ...
                            style={{ background: active ? 'rgba(37,99,235,0.15)' : 'transparent', color: active ? '#93c5fd' : '#9ca3af', borderLeft: active ? '3px solid #2563eb' : '3px solid transparent' }}>
    ...
    return (
        <div className="flex min-h-screen bg-gray-50 font-sans">
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h1 className="text-2xl font-black text-gray-900">
                                {activeTab === 'interns' ? `My Interns (${interns.length})` : 'Support Tickets'}
                            </h1>
                            <p className="text-sm text-gray-500 mt-0.5">Manage and track your assigned interns</p>
                        </div>
                        <div className="flex items-center gap-3">
                            {interns.length > 0 && (
                        ...
                        <>
                            {interns.length === 0 ? (
                                <div className="bg-white rounded-2xl border border-gray-100 py-20 text-center shadow-sm">
                                    ...
                                    <p className="font-bold text-gray-600">No interns assigned yet</p>
                                    <p className="text-sm text-gray-400 mt-1">Head HR will assign interns to you</p>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                                    {interns.map(intern => {
                                        ...
                                        return (
                                            <motion.div key={intern._id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                                                ...
                                                <div className="flex items-start gap-3 mb-4">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <p className="font-bold text-gray-900 truncate">{intern.fullName}</p>
                                                        <p className="text-xs text-gray-500 truncate">{intern.role}</p>
                                                        <p className="text-xs text-gray-400 truncate">{intern.college}</p>
                                                    </div>
                                                </div>

                                                <div className="grid grid-cols-3 gap-2 mb-4">
                                                    <div className="text-center bg-gray-50 rounded-xl p-2">
                                                        <p className="text-lg font-black text-gray-900">{tasks.length}</p>
                                                        <p className="text-[10px] text-gray-400">Tasks</p>
                                                    </div>
                                                    <div className={`text-center rounded-xl p-2 ${pendingTasks > 0 ? 'bg-blue-50' : 'bg-gray-50'}`}>
                                                        <p className={`text-lg font-black ${pendingTasks > 0 ? 'text-blue-700' : 'text-gray-900'}`}>{pendingTasks}</p>
                                                        <p className="text-[10px] text-gray-400">Review</p>
                                                    </div>
                                                    <div className={`text-center rounded-xl p-2 ${openTickets > 0 ? 'bg-amber-50' : 'bg-gray-50'}`}>
                                                        <p className={`text-lg font-black ${openTickets > 0 ? 'text-amber-700' : 'text-gray-900'}`}>{openTickets}</p>
                    ...
                    {activeTab === 'tickets' && (
                        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                                <h3 className="font-bold text-gray-900">Support Tickets <span className="text-gray-400 font-normal text-sm">({allTicketsList.length})</span></h3>
                                <span className="text-xs font-bold text-amber-600 bg-amber-50 px-2 py-1 rounded-full">{openTickets.length} open</span>
                            </div>
                                    ...
                                    {allTicketsList.map(t => (
                                        <div key={t._id} className="px-6 py-4 hover:bg-gray-50">
                                            <div className="flex flex-wrap items-center gap-2 mb-1">
                                                ...
                                                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${PRIORITY_COLORS[t.priority]}`}>{t.priority}</span>
                                            ...
                                            <p className="text-sm font-semibold text-gray-700 mb-1">{t.subject}</p>
                                            ...
                                            {t.status === 'open' && (
                                                <div className="flex gap-2">
                                                    <input
                                                        ...
                                                        className="flex-1 px-3 py-1.5 text-xs border border-gray-200 rounded-lg outline-none"
                                                        onKeyDown={e => { if (e.key === 'Enter') { handleResolveTicket(t._id, e.target.value); e.target.value = ''; } }}
                                                    />
                                                    <button onClick={e => { const inp = e.target.previousSibling; handleResolveTicket(t._id, inp.value); inp.value = ''; }}
            ...
            <AnimatePresence>
                {showMeetModal && (
                    <MeetModal interns={interns} onClose={() => setShowMeetModal(false)} />
                )}
            </AnimatePresence>
        </div>
    );
